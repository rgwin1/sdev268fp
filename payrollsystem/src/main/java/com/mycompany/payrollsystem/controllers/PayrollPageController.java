...
//        colNet.setCellValueFactory(new PropertyValueFactory<>("netPay"));
    ...
    private void onCalculatePayrollClick() throws SQLException {
        ...
        String payDate = endDate.toString();
        ...
        for (Employee emp : allEmployees) {
            ...
            if (payType.equalsIgnoreCase("Salary")) {
                grossPay = 8 * 5 * wage + ptoHours * wage;
            } else {
            ...
            PayrollRow row = new PayrollRow(id, payPeriod, totalHours + ptoHours, wage, grossPay, medicalDeduction, dependentStipend, stateTax, federalTax, socialSecurity, medicare, netPay);

            payrollTable.getItems().add(row);

            PayrollRecord record = new PayrollRecord(
                id, startDate.toString(), endDate.toString(), payDate,
                totalHours + ptoHours, overtimeHours, wage,
